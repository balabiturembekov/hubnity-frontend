...
export const useMarkNotificationAsReadMutation = (id: string) =>
  useMutation({
    mutationKey: ["markNotificationAsRead", id],
    mutationFn: () => notificationService.markOneAsRead(id),
    onMutate: async () => {
          ...
          return {
            ...
            pages: old.pages.map((page) => ({
              ...
              items: page.items.map((item) =>
                item.id === id ? { ...item, readAt: readAtNow() } : item,
              ),
            })),
          };
        },
